...
class UserService{
    all = async () => {
        try {
            const url = `${enviroment.backend}/usuario/all`
    ...
    findById = async (id) => {
        try {
            const url = `${enviroment.backend}/usuario/find-by-id/` + id
            const response = await consumeBack('GET', url);
            return response.data
        } catch (error) {
            return {
    ...
    save = async (request) => {
        try {
            const url = `${enviroment.backend}/usuario/${request.id == null ? 'create' : 'update/'+request.id}`
